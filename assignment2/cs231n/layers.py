...
def conv_backward_naive(dout, cache):
  ...
  for n in range(N):
    ...
    for f in range(F):
      for _h in range(Hout):
        for _w in range(Wout):
          wbegin, wend = stride * _w , stride * _w + WW
          hbegin, hend = stride * _h , stride * _h + HH
          dw[f] += sample[:,hbegin:hend, wbegin:wend] * dout[n,f,_h,_w]
          dxi[:,hbegin:hend, wbegin:wend] += w[f] * dout[n,f,_h,_w]

    dx[n] = dxi[:,pad:-pad,pad:-pad]
  db = np.ones_like(F) * np.sum(dout,axis=(0,2,3))
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx, dw, db
